...
import { addsoal, addSoalGroup, getDownloadURL, selectBabsoalById, uploadImageTo } from '../../testAction'

export default function Soal5({match}) {
    ...
    const addGroupSoal = async (e) => {
        ...
        setuploading(true)
        const groupNo = babsoal.list_soal.length + 1
        ...
        task.then( async (resp) => {
            ...
            dispatch(addsoal({testId, babId, data})).then( () => {
                setuploading(false)
                toogleGroupForm()
            })
        })
    ...
    const saveSoalGroup = async (e) => {
        ...
        const noSoal = babsoal.list_soal[indexSoal].soalGroup.length + 1
        ...
        task.then( async (resp) => {
            ...
            dispatch(addSoalGroup({testId, babId, data, index})).then( () => {
                setuploading(false)
                toogleSoalForm()
            })
        })
    ...
    <div className="border p-2">
        {
            babsoal.list_soal.map((opsi, index) => (
